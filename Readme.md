...
* In Listing 9.1, `FormsModule` and `HttpModule` are shown in the imports. I'm guessing these were automatically generated by the ng generator when the author wrote the chapter, but they are no longer included by default. In my chapter 8 notes I mentioned `HttpModule`, but I'm keeping in mind that `FormsModule` may need to be added later.
* In the `framework.component.html` code in listing 9.3, using the `fixed-top` class on the `nav` element results in the nav bar overlapping with the content for me. I was able to fix this by using `navbar-fixed-top` instead, which is the class used in `layout.pug`.
